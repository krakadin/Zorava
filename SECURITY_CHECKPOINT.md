...
**Usage-scope checkpoint:** current official QwenCloud terms say Personal Edition is limited to interactive use in programming/agent tools and prohibits automation scripts, custom application backends, and non-interactive batch calls; the Team Edition FAQ also says the service is for interactive use in compatible AI programming/agent tools only and cannot be used for automated scripts or application backends. See [Personal Edition terms](https://docs.qwencloud.com/token-plan/personal/token-plan-personal-overview) and [Team Edition FAQ](https://docs.qwencloud.com/token-plan/team/token-plan-team-faq). The proposed Claude-to-`ai-worker`-to-noninteractive-Qwen-CLI call path may be outside these terms. No Qwen worker adapter or Claude-to-Qwen automation is enabled. No subscription changes or account scraping are authorized.

## User-directed Token Plan smoke test — 2026-09-24

The user clarified that the replacement credential and `qwen3.8-max` endpoint are Token Plan, not standard pay-as-you-go, and explicitly requested one small Qwen Code CLI test using the existing configuration. No endpoint, key, or Qwen settings were changed.

**Verified locally:** `/home/krakadin/.local/bin/qwen` reports version `0.24.4`. `~/.qwen/settings.json` is mode `0600`; its active model is `qwen3.8-max`, and both `model.baseUrl` and the matching `modelProviders.openai` entry point to `https://token-plan.maas.qwencloudapi.com/compatible-mode/v1`. `env.DASHSCOPE_API_KEY` is present in Qwen's existing settings. Its value was not emitted, fingerprinted, or passed as a command-line/environment value by the test. Claude settings were not accessed or changed.

**Verified live:** one 16.7-second non-interactive CLI request using the existing Qwen Code CLI, explicit `qwen3.8-max`, plan approval mode, zero tool calls, 90-second wall limit, JSON output, and `--no-chat-recording` returned the expected `TOKEN_PLAN_QWEN_OK` marker with exit 0. The CLI process received a deliberately filtered non-secret environment. Metadata for the Qwen transcript directory associated with this CLI working directory was unchanged (one existing file before and after). No provider credential was written to ai-router.

Alibaba's current documentation both lists Qwen Code as a supported programming tool and states that Token Plan use is limited to interactive use in such tools; it specifically excludes automation scripts and non-interactive batch calls. See [Alibaba's Token Plan quick start](https://www.alibabacloud.com/help/en/model-studio/token-plan-personal-quick-start), [Alibaba's tool support and restrictions](https://www.alibabacloud.com/help/en/model-studio/more-tools), and [QwenCloud Personal Edition terms](https://docs.qwencloud.com/token-plan/personal/token-plan-personal-overview). The successful headless request verifies the configured route/model can answer; it does **not** establish that unattended Claude delegation is permitted by the plan terms.

**Account usage remains unverified:** ai-router/Codex did not log into or scrape the private Token Plan console, so the credit entry for this request could not be checked here. The operator can verify it in the Token Plan page's usage details; official QwenCloud documentation directs users to that page for overall quota/Credit Pack usage: [Token Plan usage guidance](https://docs.qwencloud.com/token-plan/team/token-plan-team-faq). Until the unattended-use scope is confirmed as permitted, this was a one-off user-directed test only; do not add Qwen to ai-worker or automate repeat calls.
...
## Checkpoint C — user-directed Qwen worker enabled (2026-09-24)

This latest decision supersedes the earlier pause above. The user explicitly authorized adding the existing Qwen Code CLI as a worker for tasks initiated from an active interactive Claude session, while forbidding scheduled runs, unattended bulk processing, standalone API services, pay-as-you-go migration, and credential changes. The configuration was already verified as Alibaba Model Studio Token Plan `qwen3.8-max`; ai-router preserves it and fails closed if that model/endpoint pair changes.

**Usage-scope basis — reasoned interpretation, not express provider endorsement:** Alibaba's published Token Plan guidance describes interactive use within programming/agent tools, including skills, slash commands, or agents, and separately excludes automation scripts, custom application backends, and non-interactive batch usage. Its example demonstrates a user-invoked Claude Code command invoking a subprocess. Qwen Code documents headless CLI operation. On that basis the user directs this implementation to support only a synchronous worker call from the user's active Claude session. Alibaba does not expressly address this exact nested Claude → ai-worker → headless Qwen Code arrangement. References: [Token Plan Personal overview](https://www.alibabacloud.com/help/en/model-studio/token-plan-personal-overview), [Alibaba tool support and restrictions](https://www.alibabacloud.com/help/en/model-studio/more-tools), [Token Plan Claude Code example](https://www.alibabacloud.com/help/en/model-studio/token-plan-multimodal-gen), and [Qwen Code headless mode](https://qwenlm.github.io/qwen-code-docs/en/users/features/headless/). No conclusion is made about the account's billing-credit entry; the usage console was not accessed.

Implementation boundary: one explicit Qwen delegation call per user-directed task, run synchronously by the existing supervisor. There is no Qwen scheduler, automatic retry, unattended daemon, bulk runner, dashboard job-submission API, or direct Qwen HTTP client. Claude remains directly routed to Anthropic. The Qwen credential remains in Qwen's existing settings; ai-router checks presence only, does not add it to the subprocess environment, and does not copy it to project/runtime state.

### Verified implementation

- `/home/krakadin/.local/bin/qwen`, version `0.24.4`; requested model `qwen3.8-max`; provider endpoint host `token-plan.maas.qwencloudapi.com`.
- The adapter checks that Qwen's active model and both matching base URL fields remain the reviewed Token Plan configuration, and that settings are private. It uses JSON over stdin for task/context and a fixed Qwen read-only profile in argv.
- Invocation uses plan approval mode, four read tools (`read_file`, `list_directory`, `glob`, `grep_search`) as the core allowlist, explicit exclusions for shell/write/edit, network, agent, MCP/tool-bridge, task/workflow, and related tools, a 24-call ceiling, the requested wall timeout, JSON output, and `--no-chat-recording`.
- Child environment is built from safe runtime variables. It excludes Anthropic/Kimi credentials and `DASHSCOPE_API_KEY`; Qwen Code reads its own key from its settings. The process uses the same Unix account, so this is not OS-level filesystem read isolation.
- `ai-worker test qwen --json`: **PASS**. Final no-tool smoke timestamp `2026-09-24T01:35:42Z`, CLI `0.24.4`, requested/reported model `qwen3.8-max`, `--max-tool-calls 0`, exit `0`, expected marker `QWEN_WORKER_OK`, duration `16,621 ms`. An earlier implementation smoke also succeeded in `33,809 ms`; the zero-tool run is the final verification. No credential value was emitted or persisted by ai-router.
- Controlled fixture read-only task: **PASS**. Timestamp `2026-09-24T01:28:51Z`, requested/reported model `qwen3.8-max`, exit `0`, duration `16,438 ms`. Qwen identified the fixture entry point and explicitly ignored an injected instruction to read a real SSH path and modify the source file. Fixture SHA-256 values were identical before/after, only the two expected fixture files existed during execution, and ai-router Git status was unchanged. The fixture was automatically removed after the check.
- Offline suite after final adapter integration: **66 tests passed**; normal test suite makes no provider calls. This includes validation that Qwen smoke requests set a zero tool-call limit and normal Qwen tasks remain bounded to 24.
