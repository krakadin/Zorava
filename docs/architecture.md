...
`ai-worker dashboard` starts a standard-library HTTP server on `127.0.0.1:8787`. It reads the same SQLite job state and invokes only predefined provider-test, cancellation, and retention actions. It does not accept arbitrary prompts, expose a shell, or serve arbitrary filesystem paths. Its Hosts view reports the host/controller layer — Claude Code, the Codex CLI, and the roadmap ChatGPT-hosted controller — from local metadata only (fixed executable names inside a fixed `PATH` allowlist, one bounded `--version` probe per resolved host CLI, and existence checks of two fixed delegation-skill paths), so the UI keeps the agents that delegate visibly separate from the managed Qwen/Kimi workers they hire. It is not required for synchronous host delegation; no daemon or systemd service is required.
