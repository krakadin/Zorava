...
ai-worker test qwen                 # explicit small live Token Plan usage-report request
ai-worker test kimi                 # explicit small live provider usage-report call
...
The Hosts view (`GET /api/v1/hosts`, summarized on Overview) reports the host/controller layer separately from the managed Qwen/Kimi worker cards: Claude Code and the Codex CLI as local host integrations, and the ChatGPT-hosted controller as `ROADMAP`/unavailable because that integration is only planned. Per host it shows name, role, integration kind, status, resolved executable and bounded `--version` output when available, the delegation-skill path (`~/.claude/skills/delegate-workers/SKILL.md` for Claude, `.agents/skills/delegate-workers/SKILL.md` in this checkout for Codex) and whether that fixed path exists. Detection is local and cached after the first read: two fixed executable names resolved inside a fixed `PATH` allowlist, at most one bounded `--version` probe per resolved host CLI, and `is_file()` checks of those two fixed skill paths. There is no refresh or test action, no host credential/OAuth/token file is read, skill content is never read, nothing is written to `~/.codex`, and no host agent, provider, or network endpoint is contacted.
