...
The Zorava dashboard has Overview, Jobs, Providers, Permissions, Logs, and Settings views. It shows safe provider/model/endpoint metadata, supports fixed small Qwen/Kimi usage-report tests (the provider reports its own session usage/stats in one bounded marked line; no account quota is inferred), cancellation, per-worker concurrency changes (1–4, same validation as the CLI), and a two-step retention purge. It accepts no arbitrary worker prompt or shell command. Refreshes do not contact providers. Provider cards show the installed CLI version plus cached update metadata; `Check for upgrades` is an explicit, read-only comparison against that CLI's fixed official version source, and a pending upgrade is only labeled, never installed. Test buttons track the new job through completion, display its completion timestamp and the last successful test, and refresh their action token after dashboard restarts. Model profiles can be selected on the Settings page, but only from the locally verified allowlists (Qwen Token Plan profiles on the reviewed endpoint; managed Kimi Code aliases); endpoint and credential edits are intentionally not available in the UI.

The provider cards also show usage. The Kimi card reports account quota windows (percent used, percent remaining, and reset time, with an available/stale/unavailable state) from the cache-only `QuotaCache`; `Refresh usage` is an explicit CSRF-protected action that asks the cache to read the already-running local Kimi server's usage endpoint once, subject to its 60-second cooldown — ordinary page refreshes and polling never fetch quota or start `kimi web`. Qwen has no reliable account-quota API, so its card shows only per-job token totals (input/output/cached) summed over completed jobs in retained local history and clearly states that account-level remaining quota is unavailable; no percentage is ever fabricated. Do not enter credentials into ai-router.
...
Claude's Anthropic traffic never passes through ai-worker. ai-router does not read Claude credentials, copy provider credentials, make provider inference calls itself, or provide a general-purpose shell endpoint. Its only outbound request is the dashboard's explicit CLI version check: one bounded, read-only HTTPS GET per worker to two fixed official sources (`registry.npmjs.org` for Qwen, `code.kimi.com` for Kimi), sent with no environment proxies, no redirects, no cookies, no `Authorization` or provider credential, and no caller-supplied URL. It compares versions and never installs or upgrades a CLI. Separately, the dashboard's explicit `Refresh usage` action makes one cooldown-limited loopback GET to an already-running local Kimi server's `/api/v1/oauth/usage` endpoint with the Kimi-owned local server token; the token is never shown or stored by ai-router, `kimi web` is never started, and dashboard GETs and polling never fetch quota. The worker supervisor works without the optional dashboard.
...
Live provider calls are explicit: `ai-worker test qwen` and `ai-worker test kimi` make small requests for a bounded usage/stats report. Normal tests never contact a provider.
